...
static sbc_t         sbcdecexc;
static struct sbc_priv sbcpriv;
#endif
				...
				size_t len)
{
	...
	frame->mode = (enum  sbc_frame::ech_mode)((data[1] >> 2) & 0x03);
	switch (frame->mode) {
	case SBC_MODE_MONO:
		frame->channels = 1;
		break;
	...
	frame->allocation = (enum  sbc_frame::en_allocation_method)((data[1] >> 1) & 0x01);
				...
				struct sbc_frame *frame, int ch, int blk)
{
	...
	for (idx = 0, i = 0; i < 8; i++, idx += 5) {
	 ...
	 frame->pcm_sample[ch][blk * 8 + i] += ( (frame->pcm_sample[ch][blk * 8 + i]>>1) + (frame->pcm_sample[ch][blk * 8 + i]>>2)) ;
	}
...
static inline void _sbc_analyze_eight(const int32_t *in, int32_t *out)
{
	...
	out[0] = SCALE8_STAGE2(
		MULA( _anamatrix8[6], t[0],
		MULA( _anamatrix8[3], t[1],
		MULA( _anamatrix8[0], t[2],
		MULA( _anamatrix8[2], t[3],
		MULA( _anamatrix8[7], t[4],
		MULA( _anamatrix8[4], t[5],
		MULA( _anamatrix8[1], t[6],
		MUL(  _anamatrix8[5], t[7])))))))));

	out[1] = SCALE8_STAGE2(
		MULA( -_anamatrix8[6], t[0],
		MULA( -_anamatrix8[5], t[1],
		MULA( _anamatrix8[1], t[2],
		MULA( _anamatrix8[3], t[3],
		MULA( _anamatrix8[7], t[4],
		MULA( -_anamatrix8[2], t[5],
		MULA( -_anamatrix8[0], t[6],
		...
		int output_len, int *written)
{
	...
	framelen = sbc_unpack_frame((const unsigned char *)input, &priv->frame, input_len);
	...
	if (!priv->init) {
		...
		priv->frame.codesize = sbc_get_codesize(&sbcdecexc);
		priv->frame.length = sbc_get_frame_length(&sbcdecexc);
	}
...
int sbc_encode(void * hEnc, UINT32* pnFrameSize, UINT8 * pFrame, UINT32 nRawLen, short *pRawData)    //±àÂë
{
...
//{
	...
	/* output must be large enough to receive the fd frame */
	...
	framelen = sbc_pack_frame((unsigned char *)pFrame, &priv->frame, output_len);
